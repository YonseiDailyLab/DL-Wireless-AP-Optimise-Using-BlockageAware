...
if __name__ == '__main__':
    ...
    model = Net(val_dataset.x.shape[1], 1024, 4).to(device)
    ...
    with torch.no_grad():
        for model_file in tqdm(model_files):
            ...
            y_val_pred = torch.from_numpy(scaler_y.inverse_transform(y_val_pred.cpu().numpy())).to(device, dtype=torch.float32)
